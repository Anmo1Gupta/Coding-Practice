...
public:
    ...
    Matrix(int n)
    {
        ...
        // Number the rows in the leftmost column. 0th index
        for (int i = 1; i <= n; i++)
        {
            _ptr[i][0] = i;
        }
...
int main()
{
    ...
    int input_array[no_lines][no_lines];
    ...
    { // i< no_of_queries
        ...
        {
            iss >> elem;
            input_array[i][j] = elem;
        }
    }

    MATRIX_SIZE = no_lines;
